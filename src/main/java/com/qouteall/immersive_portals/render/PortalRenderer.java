...
public abstract class PortalRenderer {
        ...
        if (client.renderViewEntity.dimension == MyRenderHelper.originalPlayerDimension) {
            ...
            return renderingPortal.canRenderEntityInsideMe(
                MyRenderHelper.originalPlayerPos.add(
                    0, client.renderViewEntity.getPosYEye(), 0
                ),
                0
            );
        }
    ...
    private Vec3d getRoughTestCameraPos() {
        return client.gameRenderer.getActiveRenderInfo().getProjectedView();
    }
    ...
    protected final void manageCameraAndRenderPortalContent(
        ...
        if (portal.getDistanceToNearestPointInPortal(client.gameRenderer.getActiveRenderInfo().getProjectedView()) > getRenderRange()) {
            return;
        }
