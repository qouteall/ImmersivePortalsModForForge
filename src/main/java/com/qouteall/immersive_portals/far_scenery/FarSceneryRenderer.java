...
public class FarSceneryRenderer {
    ...
    public static void updateCullingEquation(double distance, Direction direction) {
        Vec3d planeNormal = new Vec3d(direction.getDirectionVec());
        Vec3d cameraPos = mc.gameRenderer.getActiveRenderInfo().getProjectedView();
        
        Vec3d portalPos = planeNormal.scale(distance);
...
//        RenderSystem.disableBlend();
        ...
        bufferBuilder.pos(-1.0D, 1.0D, 1.0D).tex(1.0F, 1.0F)
            .color(255, 255, 255, 255).endVertex();
        bufferBuilder.pos(1.0D, 1.0D, 1.0D).tex(0.0F, 1.0F)
            .color(255, 255, 255, 255).endVertex();
        bufferBuilder.pos(1.0D, -1.0D, 1.0D).tex(0.0F, 0.0F)
            .color(255, 255, 255, 255).endVertex();
    ...
    private static void myBindTexture(SecondaryFrameBuffer fb) {
        ...
        GlStateManager.bindTexture(fb.fb.framebufferTexture);
