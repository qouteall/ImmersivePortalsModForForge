...
public class MixinProfileResultImpl {
     ...
     */
    @Overwrite
    public List<DataPoint> getTimings(String key) {
        ...
        List<DataPoint> list_1 = Lists.newArrayList();
        if (!key.isEmpty()) {
            key = key + '\u001e';
        }
        ...
        while (keySetIter.hasNext()) {
            ...
            if (currProfile.length() > key.length() && currProfile.startsWith(key) && currProfile.indexOf(
                ...
                list_1.add(new DataPoint(string_5, double_1, double_2, long_6));
            }
        }
        ...
        list_1.add(
            0,
            new DataPoint(key, 100.0D, (double) long_4 * 100.0D / (double) long_1, long_3)
        );
        return list_1;
    }
}
